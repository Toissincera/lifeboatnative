...
export default function MyAssignedCases({ cases }) {
  ...
  if (cases.length === 0) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80px', padding: '16px' }}>
        <Typography variant="body1">No assigned cases</Typography>
      </Box>
    );
  }
        ...
        <TableBody>
          {cases.map((caase, ix) => (
            ...
            >
              ...
              <TableCell sx={bodyCell.upda}>{dateTimeShow(caase.updated_at)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
...
const headerCell = {
  ...
  stat: { ...baseCell, minWidth: "75px", fontWeight: "bold" },
